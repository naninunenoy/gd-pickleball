...
func _init() -> void:
	...
	failed += _expect((kit.pos.z - 30.2) + 4.0 < (back.pos.z - 40.6), "kitchen camera pulls in behind the player")
	failed += _expect(kit.fov + 8.0 < back.fov, "kitchen camera FOV tightens")
	failed += _expect(absf(back.look.z - MatchRules.NVZ_SOUTH) < 12.0, "back camera looks toward the kitchen line")
	failed += _expect(kit.look.y > back.look.y, "kitchen camera looks at play height")
